...
const CATEGORIES = ["Albums", "Prints", "Frames", "Digital", "Other"];

const emptyForm = { name: "", description: "", price: "", category: "Albums", imageUrl: "" };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-semibold text-white">Product Catalogue</h2>
          <p className="text-zinc-500 text-sm">{products.length} product{products.length !== 1 ? "s" : ""} listed</p>
        </div>
        <button
      ...
      {showForm && (
        <div className="bg-zinc-900 border border-[#D4AF37]/20 rounded-lg p-6">
          <div className="flex items-center justify-between mb-5">
            <h3 className="text-white font-medium">{editId !== null ? "Edit Product" : "New Product"}</h3>
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-zinc-400 text-xs uppercase tracking-widest mb-1.5">Name *</label>
              <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required
                className="w-full px-3 py-2 bg-zinc-800 border border-zinc-700 rounded-md text-white text-sm focus:outline-none focus:border-[#D4AF37]/50" placeholder="Product name" />
            </div>
            <div>
            ...
            <div>
              <label className="block text-zinc-400 text-xs uppercase tracking-widest mb-1.5">Price (₹) *</label>
            ...
            <div className="md:col-span-2">
              ...
              <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} required rows={2}
                className="w-full px-3 py-2 bg-zinc-800 border border-zinc-700 rounded-md text-white text-sm focus:outline-none focus:border-[#D4AF37]/50 resize-none" placeholder="Product description" />
            </div>
            <div className="md:col-span-2 flex gap-3">
              <button type="submit" disabled={isSaving}
                ...
                {editId !== null ? "Save Changes" : "Create Product"}
              </button>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-zinc-500">No products yet. Add your first product above.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-3">
          {products.map((p) => (
            <div key={p.id} className="flex items-center gap-4 bg-zinc-900 border border-zinc-800 rounded-lg p-4 hover:border-zinc-700 transition-colors">
              {p.imageUrl ? (
                <img src={p.imageUrl} alt={p.name} className="w-14 h-14 rounded-md object-cover flex-shrink-0 bg-zinc-800" />
              ) : (
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-xs px-2 py-0.5 bg-zinc-800 text-zinc-400 rounded-sm flex-shrink-0">{p.category}</span>
                </div>
                <p className="text-zinc-500 text-xs truncate">{p.description}</p>
                <p className="text-[#D4AF37] text-sm font-semibold mt-1">₹{p.price.toLocaleString()}</p>
              </div>
              <div className="flex items-center gap-2 flex-shrink-0">
                <button onClick={() => startEdit(p)} className="p-2 text-zinc-500 hover:text-[#D4AF37] hover:bg-zinc-800 rounded-md transition-colors">
                ...
                {deleteId === p.id ? (
                  ...
                  <button onClick={() => setDeleteId(p.id)} className="p-2 text-zinc-500 hover:text-red-400 hover:bg-red-950/20 rounded-md transition-colors">
